...
def check_essay_with_ai(question: str, essay: str) -> dict:
    """
    ...
    try:
        ...
        logger.debug(f"Essay (truncated): {essay[:200]}...")  # לא לחשוף חיבור מלא בלוגים

        prompt = essay_test_user_prompt.format(question=question, essay=essay)
        response = prompt_llm(system_message=essay_test_system_prompt, prompt=prompt)
